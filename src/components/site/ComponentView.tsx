...
const ComponentView = ({currentView, setActive}: ComponentViewProps ) => {
  ...
  return(
    <div className={`flex flex-col rounded-[8px] h-full`}>
      <ComponentViewTab currentTab={currentTab} onCurrentTab={setCurrentTab}/>
      
      <div className={`border p-5 border-amber-600 flex-1 overflow-y-auto bg-white`}>
        {currentTab === 'Preview' ? (
          <div className="flex flex-col gap-8">
            <section className="flex flex-col gap-2">
              <div className="flex flex-col gap-2">
                <h1 className="text-2xl font-bold">{currentView.componentType}</h1>
                <p className="text-muted-foreground text-[0.95rem]">
                  {currentView.description}
                </p>
              </div>
              <PaginationArrows data={components} setActive={setActive}></PaginationArrows>
            </section>
    ...
    <div className={`flex justify-between bg-red-700 px-[8px] py-[8px] text-[0.9rem] font-medium items-center rounded-t-[8px]`}>
      <ul className={`flex gap-2`}>
        <li onClick={()=> onCurrentTab('Preview')}
            className={`px-[8px] py-[4px] rounded-[4px] cursor-pointer transition-colors ${currentTab === 'Preview' ? 'bg-blue-600 text-white' : 'hover:bg-red-600'}`}>
          Preview
        </li>
        <li onClick={()=> onCurrentTab('Code')}
            className={`px-[8px] py-[4px] rounded-[4px] cursor-pointer transition-colors ${currentTab === 'Code' ? 'bg-blue-600 text-white' : 'hover:bg-red-600'}`}>
          Code
        </li>
      </ul>
